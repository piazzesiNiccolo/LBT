...
let test_security_violation =
  QCheck2.Test.make ~name:"security violations are always captured" ~count:10000
...
let test_secure_expressions =
  QCheck2.Test.make ~name:"secure expressions never raises security violations"
    ~count:10000 ~print:Print_utils.print_exp
    (Generators.genSecureExp Generators.randomSmallSize []
       Generators.randomBaseType) (fun e -> not (is_insecure e))
;;

QCheck_runner.run_tests ~colors:true ~verbose:true
  [ test_security_violation; test_secure_expressions ]
