...
let evaluate expr =
  ...
  | Ast.Error e ->
      err_to_string e |> Printf.printf "The following error occurred:\n %s \n"
  ...
  let expr =
    Ast.Let
      ( "y",
        Ast.Eint 3,
        Ast.SandboxExecute
          ( Call (Ast.Den "fact", Ast.Den "y"),
            [ Ast.Access "fact"; Ast.Arith; Ast.Access "y" ] ) )
  in
  ...
  let fact =
    Ast.Letfun
      ( "fact",
        "n",
        Ast.If
          ( Ast.Binop (Ast.Equal, Ast.Den "n", Ast.Eint 0),
            Ast.Eint 1,
            Ast.Binop
              ( Ast.Times,
                Ast.Den "n",
